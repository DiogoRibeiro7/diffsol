...
data["month"] = (
    12.0 * (data.index.year - data.index.year.min()) + data.index.month - 1.0
)
...
data["wind_speed"] = (data["wind_speed"] - data["wind_speed"].mean()) / data[
    "wind_speed"
].std()
data["meanpressure"] = (data["meanpressure"] - data["meanpressure"].mean()) / data[
    "meanpressure"
].std()
